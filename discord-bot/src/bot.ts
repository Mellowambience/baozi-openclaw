...
async function fetchQuote(marketPda: string, side: string = 'Yes', amount: number = 1.0) {
  // Use MCP get_quote for accurate price impact and expected payout
  try {
    const res = await fetch(
      `${BAOZI_API}/api/mcp/get_quote?market=${encodeURIComponent(marketPda)}&side=${encodeURIComponent(side)}&amount=${amount}`
    );
    if (res.ok) {
      const json = await res.json() as any;
      const data = json?.data || json;
      return {
        implied_odds: data?.implied_odds ?? data?.odds ?? 0.5,
        expected_payout: data?.expected_payout ?? data?.payout ?? amount,
        price_impact: data?.price_impact ?? data?.priceImpact ?? 0,
        pool: data?.pool ?? data?.totalPool ?? 0,
      };
    }
  } catch (_) { /* fall through to local estimate */ }
  // Fallback: derive from live market data
...
async function fetchPositions(wallet: string) {
  const res = await fetch(`${BAOZI_API}/api/mcp/get_positions?wallet=${encodeURIComponent(wallet)}`);
  if (!res.ok) throw new Error(`Positions fetch error: ${res.status}`);
  const json = await res.json() as any;
  // Normalise to array of positions
  const raw = json?.data?.positions || json?.positions || json || [];
